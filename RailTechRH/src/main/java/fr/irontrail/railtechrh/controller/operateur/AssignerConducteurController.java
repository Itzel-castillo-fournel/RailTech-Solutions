...
public class AssignerConducteurController {
    ...
    public void setConducteurId(int conducteurId) {
        ...
        if (l_conducteur != null) {
            updateConducteurDisplay();
        }
    }

    private void updateConducteurDisplay() {
        try {
            if (conducteurId > 0) {
                String prenom = operateurDAO.getConducteurPrenom(conducteurId);
                String nom = operateurDAO.getConducteurName(conducteurId);
                l_conducteur.setText(prenom + " " + nom);
            } else {
                l_conducteur.setText("Aucun conducteur sélectionné");
            }
        } catch (SQLException e) {
            e.printStackTrace();
            l_conducteur.setText("Erreur lors du chargement du conducteur");
        }
    }
    ...
    public void b_onClickAssignDriver(ActionEvent actionEvent) throws SQLException {
        if (conducteurId <= 0) {
            showTemporaryMessage(l_noTrajetFound, "Erreur : Aucun conducteur sélectionné.", Paint.valueOf("red"), 10);
            return;
        }
